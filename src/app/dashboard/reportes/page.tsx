...
import { Search, ChevronLeft, ArrowDownToLine, SlidersHorizontal } from "lucide-react";
...
export default function ReportesPage() {
  ...
  const filteredEmpleados = REPORTES_MOCK.filter(
    (emp) =>
  ...
  return (
    <div className="min-h-screen bg-white p-8 font-sans">
      <div className="max-w-6xl mx-auto">
        ...
        {/* Título */}
        <h1 className="text-4xl font-bold text-[#150AB4] mb-6">Reportes</h1>
        ...
        {/* Tarjetas KPI con diseño "hoja" y píldora superior */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-10">
          
          {/* Card 1: Total de reportes */}
          <div className="bg-[#e0fcf2] p-6 rounded-tl-[2rem] rounded-br-[2rem] rounded-tr-xl rounded-bl-xl shadow-sm border border-[#c2f7e4] relative">
            <div className="w-20 h-2 bg-[#34d399] rounded-full mb-4"></div>
            <h3 className="text-sm font-bold text-gray-800 mb-2">Total de reportes</h3>
            <p className="text-3xl font-bold text-[#34d399] mb-1">S/ 100.00</p>
          </div>
          ...
          <div className="bg-[#e8fce8] p-6 rounded-tl-[2rem] rounded-br-[2rem] rounded-tr-xl rounded-bl-xl shadow-sm border border-[#c1f7c1] relative">
            <div className="w-20 h-2 bg-[#4ade80] rounded-full mb-4"></div>
          ...
          {/* Card 3: AFP/ONP */}
          <div className="bg-[#f0f4ff] p-6 rounded-tl-[2rem] rounded-br-[2rem] rounded-tr-xl rounded-bl-xl shadow-sm border border-[#d6e0ff] relative">
            <div className="w-20 h-2 bg-[#818cf8] rounded-full mb-4"></div>
          ...
          {/* Card 4: SCTR */}
          <div className="bg-[#fffee6] p-6 rounded-tl-[2rem] rounded-br-[2rem] rounded-tr-xl rounded-bl-xl shadow-sm border border-[#fef08a] relative">
            <div className="w-20 h-2 bg-[#facc15] rounded-full mb-4"></div>
        ...
        {/* Herramientas de la tabla (Buscador, Filtrar, Exportar) */}
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-left text-sm whitespace-nowrap">
              <thead className="bg-[#3b36b5] text-white">
                <tr>
                  <th className="p-4 font-semibold rounded-tl-xl text-center w-64">Empleado</th>
                  <th className="p-4 font-semibold text-center">DNI</th>
                  <th className="p-4 font-semibold text-center">Cargo</th>
                  <th className="p-4 font-semibold text-center">Sueldo base</th>
                  <th className="p-4 font-semibold text-center">Bonos</th>
                  <th className="p-4 font-semibold rounded-tr-xl text-center">Total</th>
                </tr>
              </thead>
              <tbody>
                {filteredEmpleados.map((emp, idx) => (
                  ...
                  >
                    ...
                    <td className="p-4 font-semibold text-gray-800 text-center">{emp.dni}</td>
                    <td className="p-4 font-semibold text-gray-800 text-center">{emp.cargo}</td>
                    <td className="p-4 font-semibold text-gray-800 text-center">{emp.sueldoBase}</td>
                    <td className="p-4 font-semibold text-gray-800 text-center">{emp.bonos}</td>
