...
export default function IdentificacionBasica({ nombre, apellidoPaterno, apellidoMaterno, numeroDocumento, tipoDocumento, fechaVinculacion, fechaRetiro, cargo, onChange }: Props) {
  const tipoDocumentoLabel = tipoDocumento === 'carnet' ? 'Carnet Extranjeria' : (tipoDocumento === 'dni' ? 'DNI' : tipoDocumento);
  const cargoLabel = cargo === 'empleador' ? 'Empleador' : cargo === 'ejecutivo' ? 'Ejecutivo' : cargo === 'obrero' ? 'Obrero' : 'No seleccionado';
    ...
    <>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        ...
        <div>
          ...
          <Input className="bg-gray-200 h-10 w-full" value={apellidoPaterno} disabled />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Apellido Materno</label>
          <Input className="bg-gray-200 h-10 w-full" value={apellidoMaterno} disabled />
        </div>
      </div>
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        <div>
          ...
          <Input className="bg-gray-200 h-10 w-full" value={tipoDocumentoLabel} disabled />
        </div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">N° de Documento*</label>
          <Input className="bg-gray-200 h-10 w-full" value={numeroDocumento} disabled />
        </div>
        <div></div>
        <div>
          <label className="block text-sm font-medium text-gray-700 mb-2">Cargo</label>
          <Input className="bg-gray-200 h-10 w-full" value={cargoLabel} disabled />
        </div>
      </div>
