...
export function ObreroForm({ onBack, personalForm, cargo }: ObreroFormProps) {
  ...
  const familiaresValidos =
    !asignacionFamiliar || validateFamiliares(familiares);
    ...
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <IdentificacionBasica
        ...
        onChange={(field, value) =>
          setValue(field as keyof ObreroSchemaType, value, {
            shouldValidate: true,
          })
        }
      />
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        ...
        <Field label="Turno" required error={errors.turno?.message}>
          <Select
            value={values.turno}
            onValueChange={(value) =>
              setValue("turno", value, { shouldValidate: true })
            }
          >
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        ...
        <Field
          label="Proyectos Asignado"
          required
          error={errors.proyectosAsignado?.message}
        >
          <Input
            className="bg-white h-10 w-full"
            {...register("proyectosAsignado")}
          />
        </Field>
        <Field
          label="Clasificación de Riesgos"
          required
          error={errors.clasificacionRiesgos?.message}
        >
          <Select
            value={values.clasificacionRiesgos}
            onValueChange={(value) =>
              setValue("clasificacionRiesgos", value, { shouldValidate: true })
            }
          >
            <SelectTrigger className="bg-white h-10 w-full">
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        ...
        >
          <Select
            ...
            onValueChange={(value) =>
              setValue("tipoContrato", value, { shouldValidate: true })
            }
          >
            <SelectTrigger className="bg-white h-10 w-full">
      ...
      <div className="grid grid-cols-4 gap-4 mb-6">
        ...
        <Field
          ...
          required
          error={errors.regimenPensionario?.message}
        >
          <Input
            className="bg-white h-10 w-full"
            {...register("regimenPensionario")}
          />
        </Field>
      ...
      <div className="flex justify-between mt-8">
        ...
        <Button
          type="submit"
          disabled={!canSubmit}
          className={`px-12 h-10 text-white ${
            canSubmit
              ? "bg-[#150AB4] hover:bg-[#0F088A]"
